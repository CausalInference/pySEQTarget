...
def _diagnostics(self):
    ...
    out = {"unique_outcomes": unique_out,
           "nonunique_outcomes": nonunique_out}
    
    if self.method == "censoring":
        unique_switch = _switch_diag(self, unique = True)
        nonunique_switch = _switch_diag(self, unique = False)
        out.update({"unique_switches": unique_switch,
                    "nonunique_switches": nonunique_switch})
...
def _outcome_diag(self, unique):
    if unique:
        data = self.DT.select([
            ...
            self.treatment_col,
            self.outcome_col
        ]).group_by(self.id_col).last()
    else: 
        data = self.DT
    out = data.group_by([self.treatment_col, 
                         self.outcome_col]).len()
    
    return out

def _switch_diag(self, unique):
    if not self.excused:
        data = self.DT.with_columns(pl.lit(False)
                                    .alias("isExcused"))
    else:
        data = self.DT
