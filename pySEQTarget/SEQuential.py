...
class SEQuential:
    ...
    def bootstrap(self, **kwargs) -> None:
        ...
        UIDs = (
            self.DT.select(pl.col(self.id_col))
            .unique()
            .sort(self.id_col)
            .to_series()
            .to_list()
    ...
    def fit(self) -> None:
        ...
        # Fix the knots of any cr(x, df = N) spline term — whether written into
        # a formula by hand or generated by weight_spline — from the full data
        # each model is fit on, so every bootstrap replicate builds the same
        # basis as the main fit rather than one memorised from its own rows.
        if boot_idx is None:
