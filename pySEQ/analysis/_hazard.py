...
def _hazard_handler(self, data, idx, boot_idx, rng):
    exclude_cols = ["followup", f"followup{self.indicator_squared}", self.treatment_col,
                    f"{self.treatment_col}{self.indicator_baseline}", "period", self.outcome_col]
    ...
    trials = (
        ...
        .group_by([self.id_col, "trial"]).first()
        .with_columns([pl.lit(list(range(self.followup_max + 1))).alias("followup")])
        .explode("followup")
        .with_columns([(pl.col("followup") ** 2).alias(f"followup{self.indicator_squared}")])
    )
    ...
    if ce_model is not None:
        sim_data = sim_data.with_columns([
            pl.when(pl.col("outcome") == 1).then(pl.lit(1))
            .when(pl.col("ce") == 1).then(pl.lit(2))
            .otherwise(pl.lit(0)).alias("event")
        ])
    else:
    ...
    try:
        ...
        if ce_model is not None:
            ...
            cph = CoxPHFitter()
            cph.fit(cox_data, duration_col='followup', event_col='event_binary',
                   formula=f"`{self.treatment_col}{self.indicator_baseline}`")
        else:
            cph = CoxPHFitter()
            cph.fit(sim_data_pd, duration_col='followup', event_col='event',
                   formula=f"`{self.treatment_col}{self.indicator_baseline}`")
