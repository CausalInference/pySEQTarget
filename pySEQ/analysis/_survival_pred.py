...
def _calculate_risk(self, data, idx=None, val=None):
    ...
    for treatment_val in self.treatment_level:
        TxDT = SDT.with_columns([
            pl.lit(treatment_val)
             .alias(f"{self.treatment_col}{self.indicator_baseline}")
             ])
        
        if self.method == "dose-response":
            if treatment_val == self.treatment_level[0]:
                TxDT = TxDT.with_columns([
                    pl.lit(0.0).alias("dose"),
                    pl.lit(0.0).alias("dose_sq")
                ])
            else:
                TxDT = TxDT.with_columns([
        ...
        if self.compevent_colname is not None:
            ...
            TxDT = TxDT.group_by("followup").agg([pl.col(col).mean() for col in surv_names + inc_names]).sort("followup")
            main_col = "surv"
            boot_cols = [col for col in surv_names if col != "surv"]
        else:
            TxDT = (
                TxDT.with_columns([(1 - pl.col(col))
                                   .cum_prod()
                                   .over("TID")
                                   .alias(col) for col in outcome_names
                                   ])
                .group_by("followup").agg([pl.col(col).mean() for col in outcome_names])
                .sort("followup")
        ...
        if boot_cols:
            ...
            if self.compevent_colname is not None:
                ...
                if inc_boot_cols:
                    inc_risk = (
                        ...
                        .unpivot(index="followup", on=inc_boot_cols, variable_name="bootID", value_name="inc_val")
                        .group_by("followup").agg([
                    ...
                    final_cols = ["followup", main_col, "SE", "LCI", "UCI", "inc", "inc_SE", "inc_LCI", "inc_UCI"]
                    risk = risk.select(final_cols).with_columns(pl.lit(treatment_val).alias(self.treatment_col))
                    ...
                    risk = risk.select(["followup", main_col, "SE", "LCI", "UCI"]).with_columns(
                        pl.lit(treatment_val)
                ...
                risk = risk.select(["followup", main_col, "SE", "LCI", "UCI"]).with_columns(pl.lit(treatment_val).alias(self.treatment_col))
                fup0 = pl.DataFrame({
                    "followup": [0],
                    main_col: [fup0_val],
                    "SE": [0.0],
            ...
            fup0_val = 1.0 if self.compevent_colname else 0.0
            risk = TxDT.select(["followup", main_col]).with_columns(pl.lit(treatment_val).alias(self.treatment_col))
            fup0 = pl.DataFrame({
                "followup": [0],
                main_col: [fup0_val],
                self.treatment_col: [treatment_val]
    ...
    if self.compevent_colname is not None:
        ...
        surv_out = out.select(surv_cols).rename({"surv": "pred"}).with_columns(pl.lit("survival").alias("estimate"))
        
        risk_cols = ["followup", self.treatment_col, (1 - pl.col("surv")).alias("pred")]
        if has_ci:
            risk_cols.extend([pl.col("SE"), (1 - pl.col("UCI")).alias("LCI"), (1 - pl.col("LCI")).alias("UCI")])
        ...
        if has_ci:
            inc_cols.extend([pl.col("inc_SE").alias("SE"), pl.col("inc_LCI").alias("LCI"), pl.col("inc_UCI").alias("UCI")])
        inc_out = out.select(inc_cols).with_columns(pl.lit("incidence").alias("estimate"))
        
        out = pl.concat([surv_out, risk_out, inc_out])
    else:
        out = out.rename({"pred_outcome": "pred"}).with_columns(pl.lit("risk").alias("estimate"))
