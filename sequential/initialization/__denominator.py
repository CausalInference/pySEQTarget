def __denominator(self) -> str:
    ...
    tv_bas = "+".join([f"{v}{self.indicator_baseline}" for v in self.time_varying_cols]) if self.time_varying_cols else None
    ...
    if self.weight_preexpansion:
        if self.method == "dose-response":
            ...
            out = "+".join(filter(None, [fixed, tv, time]))
        elif method == "censoring" and self.excused:
            out = "+".join(filter(None, [fixed, tv, time]))
    else:
        if self.method == "dose-response":
