...
def test_ITT_coefs():
    ...
    expected = [-6.828506035553407, 0.18935003090041902, 0.12717241010542563, 
...
def test_PreE_dose_response_coefs():
    ...
    s.fit()
    matrix = s.outcome_model[0]['outcome'].summary2().tables[1]["Coef."].to_list()
    expected = [-4.842735939069144, 0.14286755770151904, 0.055221018477671927, 
...
def test_PostE_dose_response_coefs():
    ...
    s.fit()
    matrix = s.outcome_model[0]['outcome'].summary2().tables[1]["Coef."].to_list()
    expected = [-6.265901713761531, 0.14065954021957594, 0.048626017624679704, 
...
def test_PreE_censoring_coefs():
    ...
    s.fit()
    matrix = s.outcome_model[0]['outcome'].summary2().tables[1]["Coef."].to_list()
    expected = [-4.818288687908951, 0.511665606890965, 0.062028316788368384, 
...
def test_PostE_censoring_coefs():
    ...
    s.fit()
    matrix = s.outcome_model[0]['outcome'].summary2().tables[1]["Coef."].to_list()
    expected = [-7.9113179326280445, 0.49092190701455873, 0.08903087485402544, 
...
def test_PreE_censoring_excused_coefs():
    ...
    s.fit()
    matrix = s.outcome_model[0]['outcome'].summary2().tables[1]["Coef."].to_list()
    expected = [-6.175691049418418, 1.3493634846413598, 0.1072284696749134, 
...
def test_PostE_censoring_excused_coefs():
    ...
    s.fit()
    matrix = s.outcome_model[0]['outcome'].summary2().tables[1]["Coef."].to_list()
    expected = [-7.126398786875262, 0.2632047482928519, 0.13345454814736696,
...
def test_PreE_LTFU_ITT():
    ...
    s.fit()
    matrix = s.outcome_model[0]['outcome'].summary2().tables[1]["Coef."].to_list()
    expected = [-21.640523091572796, 0.0685235184372898, -0.19006360662228572,
...
def test_ITT_multinomial():
    ...
    s.fit()
    matrix = s.outcome_model[0]['outcome'].summary2().tables[1]["Coef."].to_list()
    expected = [-47.505262164163625, 1.76628017234151, 22.79205044396338, 
...
def test_weighted_multinomial():
   ...
   s.fit()
   matrix = s.outcome_model[0]['outcome'].summary2().tables[1]["Coef."].to_list()
   expected = [-111.35419661939163, -12.571187230338328, 9.234157699403015, 
